...
## 6. LOW_BATTERY i RETURN_TO_BASE

U ovoj verziji je dodata logika za povratak u bazu kada baterija padne ispod 20%.

Tok je:

```text
dron → ALARM LOW_BATTERY → regionalni server → centralni server
centralni server → RETURN_TO_BASE → regionalni server → dron
```
...
Centralni server na osnovu `REGION_ID` pronalazi bazne koordinate regionalnog servera i vraća komandu:
...
Regionalni server ovu komandu prosljeđuje dronu.

Dron po prijemu komande:
1. mijenja status u `RETURN_TO_BASE`;
2. briše aktivnu rutu lokalno;
3. prelazi na koordinate baze;
4. šalje `ACK_RTB`.

Centralni server aktivnu misiju tog drona označava kao:

```text
ABORTED_LOW_BATTERY
```

Pošto se konflikt ruta provjerava samo nad misijama sa `status = 'ACTIVE'`, ova ruta i visinski slot se oslobađaju za druge dronove.
